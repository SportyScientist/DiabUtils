...
classify_glycemia <- function(glucose_000, glucose_120, hba1c = NULL, system = "ADA", 
  ...
  # Input checks
  if (!is.numeric(glucose_000) || glucose_000 <= 0) {
    stop("Error: 'glucose_000' must be a positive numeric value.")
  }
  if (!is.numeric(glucose_120) || glucose_120 <= 0) {
  ...
  # ADA Classification
  if (system == "ADA") {
    if (!is.null(hba1c) && hba1c >= 6.5) {
      result <- "DIA"
    } else if (glucose_000 >= 126 || glucose_120 >= 200) {
      result <- "DIA"

    } else if ((glucose_000 >= 100 && glucose_000 < 126) || 
               (glucose_120 >= 140 && glucose_120 < 200) || 
               (!is.null(hba1c) && hba1c >= 5.7 && hba1c < 6.5)) {
      if (details) {
  ...
  # WHO Classification
  if (system == "WHO") {
    if (glucose_000 >= 126 || glucose_120 >= 200) {
      result <- "DIA"
